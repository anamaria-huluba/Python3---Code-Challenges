...
This shows the structure of a simple class which only contains instance variables. The three instance variables are set to 0 for now, which means that they can only 
be changed by manually by accessing them from an object of the DriveBot class.

2. Adding Robot Logic

Now we want to add some logic to our robot. It would be very useful to be able to control our robot, say we are going to create a control_bot method which changes 
the speed and direction. We are also going to create a method called adjust_sensor. This method is used to change the range of our robot’s sensors which are used to 
detect obstacles. 
...
These two methods were added inside of the DriveBot class. They are used to replace the instance variables with new values from the input parameters. We use 
self.variable_name to access a certain instance variable within the class.
...
This upgraded constructor includes input parameters as well as default values for those parameters. This means that if no value is provided for those parameters, 
then the value they are set equal to will be used. Here are some examples of different ways I used the constructor:
...
We want to add a new feature which allows the use to control multiple robots at once. The robots should be able to all have the same latitude and longitude GPS 
destination coordinates as well as a setting for disabling them all called all_disabled. We can accomplish this using class variables. 
...
I placed the class variables at the top of the class outside of the constructor. These variables can be accessed within the scope of the entire class. This means that 
the class variables contained within every object from the DriveBot class will change if we modify the class variable directly. 
...
In order to keep track of the robots we are creating, we want to be able to assign an ID value to each robot when it is created. If we create five robots in a row, we 
want the IDs of each robot to be 1, 2, 3, 4, and 5 respectively. This can be accomplished by using a class variable counter which increments and is assigned to an 
instance variable for the ID whenever the constructor is called. 
...
This information is shared across all robot objects we create from the class. Every time a robot object is created, the constructor is called and the count is 
incremented. Each robot has an instance variable for id which is local to that robot object. This is assigned at the time of construction and stores what the count 
was at that time.

Python Code Challenges: Classes (Advanced)

You have created a robotics company which began with four-wheeled driving robots, but you are now expanding the technology to include bipedal robots. Since a lot of 
robotic logic already exists from the driving robot, we can modularize the robot class structure using object oriented programming concepts such as inheritance. These
five advanced challenge problems will test your knowledge of classes and interactions between multiple classes.

1. Robot Inheritance

The generic variables and methods have been placed into a Robot class for you. You will need to create two new classes. One for DriveBot and one for WalkBot. Both of 
these should inherit from the Robot class. The constructor for the DriveBot will be essentially the same as the superclass constructor, but the WalkBot constructor 
will include an extra parameter for an instance variable called step_length. 
...
By placing Robot in the parentheses next to the two new classes, we cause them to inherit from the Robot class. We can use super().__init__(self, param_1, param_2, 
etc...) to call the superclass constructor. This will use the constructor in the Robot class to assign the values to the instance variables. Any new changes can then 
be added afterward.
...
As you can see, in order to override the method, we can re-define it again with the same name. You can use super().method_name() in order to call the superclass 
...
This method shows how the superclass method can be called conditionally. In this case it depends on the speed of the robot and if an obstacle was found. We use modulus
when rotating the robot to loop back past 0 if we go over 360 degrees. This is because the robot would have made a full circle.
...
decrease the speed using the + and - operators. For the DriveBot class, we want to adjust the speed and sensor_range with those operators. Lastly, for the WalkBot 
class, we want to adjust the speed and step_length with those operators. We can override the dunder methods which represent those operations and call the superclass 
version of those dunder methods. 
...
In the WalkBot class, override the + operator to call the superclass version of the operator as well as add half of the value on the right side of the operator to the
...
The above code only shows the dunder methods which were added to each class. The child classes both override those operations as well. This shows how superclass 
versions of dunder methods can also be called from a child class.

5. Prevent A Robot Takeover

Finally, we are going to make some last additions to our bipedal robot and test it out. Your customers are worried that your new robot model might become sentient 
when controlled in large numbers. To prevent this, program the bipedal robots so that if five WalkBots are created when there are already at least 5 DriveBots 
(causing 10 or more total bots) then disable all robots. 
...
Also, in the constructor, check if the number of total Robots (from the superclass) is greater than or equal to 10 and if the number of WalkBots is greater than or
equal to 5.
...
This modification uses a similar method of counting from the Robot class, but it uses a separate counter to only track the number of WalkBots. Whenever a new WalkBot 
is created, the constructor is called and the two counts are checked. If the total number of robots is at least 10 and the number of WalkBots reaches 5 then all 
robots are disabled.
